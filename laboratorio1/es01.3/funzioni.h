...
void SumErrProg (int  M, int N, double* sum1, double l, double d, const char* filename){
   ...
   for(int i=0; i<N; i++){ //inizializzo gli array
    ...
    for(int i=0; i<N; i++){
      ave[i] = L*2*l/(sum1[i]*d);   //ogni elemento di sum1 è stato riempito con L numeri random. Qui calcolo la media degli L numeri random per ogni blocco      
      av2[i] = pow((ave[i]),2);  
    }
