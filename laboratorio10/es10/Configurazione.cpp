...
configurazione::configurazione( ){ //costruttore
...
int configurazione::getm_c(int i, int j){
...
void configurazione::setm_c(int i, int j, int value){
...
configurazione configurazione:: operator=( configurazione & copy){ //overload operatore =
	//configurazione copy(c);
	for(int i=0; i< m_N; i++){
       for(int j=0; j< m_N; j++){
...
void configurazione:: permutation(int i, int j){ //metodo che permuta le città i e j
...
void configurazione:: permutation_contiguous(int m, int k, int l){ //metodo che permuta le città dalla k-esima alla k+m esima con quelle dalla l+k-esima alla l+m+k esima
    if(k+m < l){
	   for (int i=k; i<k+m; i++){
          permutation(i, l+i);
	   }
...
void configurazione:: crossover( configurazione & copy, int cut, int  index1[],int index2[] ){ //metodo che applica il crossover a due città
	//configurazione copy(c);
	int conta=0;
	for(int i=0; i< m_N; i++){
       for(int j=0; j< cut; j++){
...
void configurazione:: shift(int m, int n, int k){ //metodo che sposta di n posizioni m città dalla k esima città
...
void configurazione:: inversion(int m, int k){ //metodo che inverte m città a partire dalla k-esima
       int conta=0;
	   int km=0;
 ...
 void configurazione:: check(){ //metodo di controllo
	 int a=0;
	 int v[m_N];
...
void configurazione::print(){ //metodo che stampa la matrice del percorso
	for (int i=0; i<m_N; i++){
	cout << endl;
		for (int j=0; j<m_N; j++){
