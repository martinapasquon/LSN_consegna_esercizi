...
//Read initial configuration
  ...
  if(restart)
  {
    ReadConf.open("./config_MD/config_liquid.out");
    ReadVelocity.open("./config_MD/velocity_liquid.out");
  ...
  for (int i=0; i<npart; ++i)
  {
    if(iNVET) //non ci serve ora
    {
      xold[i] = x[i];
      yold[i] = y[i];
      zold[i] = z[i];
    }
    else // questo si invece
    {
...
// distance i-j in pbc
      ...
      if(dr < rcut)
      {
        ...
        v += vij;
        prij = 1.0/pow(dr,12) - 1.0/(2.0*pow(dr,6));
        pr+=prij;
      }
      i_r=dr/(box/(2.*nr)); //divido la metà del raggio totale (box) in nr intervalli e incremento g_r di 2 all'indice i_r
      //cout << i_r << "  "<< nr << "  "<< dr << "  " << box/(2.) << endl;
      if (dr<box/(2.)) //i_r deve essere minore di nr
         g_r[i_r]+=2;
    }          
  }
  ...
  walker[ip] = rho *(2.0 / 3.0) * kin/(double)npart  + 48.0 * pr/(3*vol); //Pressure
...
void Averages(int iblk) //Print results for current block
{
   ...
   ofstream Epot, Ekin, Etot, Temp, Press, Gr;
   const int wd=20;
    
   // cout << "Block number " << iblk << endl;
   // cout << "Acceptance rate " << accepted/attempted << endl << endl;
    
    Epot.open("output_epot_liquid_MD.txt",ios::app);
    Ekin.open("output_ekin_liquid_MD.txt",ios::app);
    Temp.open("output_temp_liquid_MD.txt",ios::app);
    Etot.open("output_etot_liquid_MD.txt",ios::app);
    Press.open("output_ptot_liquid_MD.txt",ios::app);
    Gr.open("output_gr_liquid_MD.txt",ios::app);
    
    double tail_corr_U=(8./.9)*rho*M_PI*(pow(rcut, -9))-(8./3.)*rho*M_PI*(pow(rcut,-3)); 
    stima_pot = blk_av[iv]/blk_norm/(double)npart + tail_corr_U; //Potential energy
    ...
    double tail_corr_P=32*M_PI*rho*(pow(3*rcut, -9)-(pow(2*rcut,-3)));
    stima_press = blk_av[ip]/blk_norm  +  tail_corr_P; //Pessure
    ...
    for(int k=0; k<nr; k++){
      stima_g_r[k] = blk_av_g[k]/(blk_norm*rho*npart*(4./3.)*M_PI*(pow((k+1)*delta_r, 3)-pow((k)*delta_r, 3))); 
      glob_av_g[k] += stima_g_r[k];
      glob_av2_g[k] += stima_g_r[k]*stima_g_r[k];
      err_g_r[k]=Error(glob_av_g[k],glob_av2_g[k],iblk);
    } 
...
//Pressure
   ...
   for(int k=0; k<nr; k++){
   Gr << setw(wd) << iblk << "  "<< k <<  setw(wd) << stima_g_r[k] << setw(wd) << glob_av_g[k]/(double)iblk << setw(wd) << err_g_r[k] << endl;
}
...
void ConfFinal(void)
{
  ...
  WriteConf.open("config_liquid_MD.out");
  WriteVelocity.open("velocity_liquid_MD.out");
  for (int i=0; i<npart; ++i)
  {
