...
//Read initial configuration
  ...
  if(restart)
  {
    ReadConf.open("config_gas.out");
    ReadVelocity.open("velocity_gas.out");
...
void Averages(int iblk) //Print results for current block
{
   ...
   const int wd=12;
    ...
    Epot.open("output_epot_gas.txt",ios::app);
    Ekin.open("output_ekin_gas.txt",ios::app);
    Temp.open("output_temp_gas.txt",ios::app);
    Etot.open("output_etot_gas.txt",ios::app);
    Press.open("output_ptot_gas.txt",ios::app);
    ...
    stima_press = blk_av[ip]/blk_norm; //Pessure
...
//Temperature
    Temp << iblk << " "<< stima_temp << setw(wd) << glob_av[it]/(double)iblk << setw(wd) << err_temp << endl;
//Pressure
...
void ConfFinal(void)
{
  ...
  WriteConf.open("config_gas.out");
  WriteVelocity.open("velocity_gas.out");
  for (int i=0; i<npart; ++i)
  {
